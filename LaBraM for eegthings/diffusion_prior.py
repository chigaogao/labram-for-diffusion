...
class DiffusionPriorUNet(nn.Module):
    ...
    def forward(self, x, t, c=None):
        ...
        # 1. time embedding
        t = self.time_proj(t) # (batch_size, time_embed_dim)#1024x512
        ...
        # 3.1 input
        x = self.input_layer(x) #x:1024x1024
...
class Pipe:
    ...
    def train(self, dataloader, num_epochs=10, learning_rate=1e-4):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                ...
                noise = torch.randn_like(h_embeds)#1024x1024

                # 3. sample timestep
                timesteps = torch.randint(0, num_train_timesteps, (N,), device=device)#1024
                ...
                # 5. predict noise
                noise_pre = self.diffusion_prior(perturbed_h_embeds, timesteps, c_embeds)#向图像嵌入添加噪声，利用脑电特征作为条件，引导去噪声
    ...
    def generate(
        ...
        if c_embeds is not None:
            c_embeds = c_embeds.to(self.device)#eeg_embedding

        # 3. Prepare noise
        h_t = torch.randn(N, self.diffusion_prior.embed_dim, generator=generator, device=self.device)#噪声1x1024
        ...
        for _, t in tqdm(enumerate(timesteps)):
            ...
            if guidance_scale == 0 or c_embeds is None:
                ...
                noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)#有条件＋无条件
...
if __name__ == '__main__':
    ...
    eeg_features_train = torch.randn(2048,1024)
    emb_img_train_4 = torch.randn(2048,1024)
