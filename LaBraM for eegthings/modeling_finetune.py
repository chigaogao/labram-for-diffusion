...
class TemporalConv(nn.Module):
    ...
    def forward(self, x, **kwargs):
        x = rearrange(x, 'B N A T -> B (N A) T')#4x64x4x200->4x256x200
        B, NA, T = x.shape#4x256x200
        x = x.unsqueeze(1)#4x1x256x200
        x = self.gelu1(self.norm1(self.conv1(x)))#4x8x256x25
        x = self.gelu2(self.norm2(self.conv2(x)))#4x8x256x25
        x = self.gelu3(self.norm3(self.conv3(x)))#4x8x256x25
        x = rearrange(x, 'B C NA T -> B NA (T C)')#4x256x200
        return x
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
        ...
        self.num_features = self.embed_dim = embed_dim#200  # num_features for consistency with other models
        ...
        self.blocks = nn.ModuleList([
            ...
            for i in range(depth)])#12个block
        self.norm = nn.Identity() if use_mean_pooling else norm_layer(embed_dim)
        self.fc_norm = norm_layer(embed_dim) if use_mean_pooling else None
        self.head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()#分类头

        if self.pos_embed is not None:
            trunc_normal_(self.pos_embed, std=.02)
        if self.time_embed is not None:
            trunc_normal_(self.time_embed, std=.02)
        trunc_normal_(self.cls_token, std=.02)#使用一个标准差为 0.02 的截断正态分布来初始化
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        batch_size, n, a, t = x.shape#64x63x1x200
        input_time_window = a if t == self.patch_size else t#
        x = self.patch_embed(x)#64x63x200

        cls_tokens = self.cls_token.expand(batch_size, -1, -1) #64x1x200 # stole cls_tokens impl from Phil Wang, thanks

        x = torch.cat((cls_tokens, x), dim=1)#64x64x200
