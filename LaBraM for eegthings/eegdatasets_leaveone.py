...
model_type = 'ViT-H-14'
import open_clip
local_model_path = "/root/autodl-fs/models--laion--CLIP-ViT-H-14-laion2B-s32B-b79K/snapshots/1c2b8495b28150b8a4922ee1c8edee224c284c0c/open_clip_pytorch_model.bin"
vlmodel, preprocess_train, feature_extractor = open_clip.create_model_and_transforms(
    model_type, pretrained=local_model_path, precision='fp32', device = device)
...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, val_size=None):
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = "/root/autodl-tmp/EEG_Image_decode-main/LaBraM for eegthings/ViT-H-14_features_train.pt" if self.train else  "/root/autodl-tmp/EEG_Image_decode-main/LaBraM for eegthings/ViT-H-14_features_test.pt"
            #vit_h_14_features_train.pt
    ...
    def load_data(self):
        ...
        dirnames = [d for d in os.listdir(directory) if os.path.isdir(os.path.join(directory, d))]#1654个文件夹
        dirnames.sort()
        ...
        for dir in dirnames:
            ...
            texts.append(new_description)#1654个 this picture is []
        ...
        for subject in self.subjects:
            if self.train:
                ...
                file_path = os.path.join(self.data_path, subject, file_name)#"/root/autodl-tmp/Preprocessed_data_250Hz/sub-01/eeg_training"
                data = np.load(file_path, allow_pickle=True)

                preprocessed_eeg_data = torch.from_numpy(data['preprocessed_eeg_data']).float().detach()  #16540x4x63x250
                times = torch.from_numpy(data['times']).detach()[40:]
                ...
                if self.classes is not None and self.pictures is not None:
                    ...
                    for i in range(n_classes):#0-1654
                        start_index = i * samples_per_class
                        # if self.exclude_subject==None:
                        #     preprocessed_eeg_data_class = preprocessed_eeg_data[start_index: start_index+samples_per_class]
                        # else:
                        preprocessed_eeg_data_class = preprocessed_eeg_data[start_index: start_index+samples_per_class]#10x4x63x250
                        ...
                        # print("preprocessed_eeg_data_class", preprocessed_eeg_data_class.shape)
                        labels = torch.full((samples_per_class,), i, dtype=torch.long).detach()  #十个0 十个1 ...十个1654
                        data_list.append(preprocessed_eeg_data_class)
                        label_list.append(labels)
        ...
        if self.train:
            ...
            data_tensor = torch.cat(data_list, dim=0).view(-1, *data_list[0].shape[2:])#66160x63x250
    ...
    def __getitem__(self, index):
        ...
        x = self.data[index]#test时 不打乱顺序 一个一个的取
