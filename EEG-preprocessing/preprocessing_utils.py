def epoching(args, data_part, seed):
	...
	for s in range(args.n_ses):
		...
		eeg_dir = os.path.join('sub-01', 'sub-'+
		...
		events = mne.find_events(raw, stim_channel='stim')#4080
		...
		idx_target = np.where(events[:,2] == 99999)[0]#总共有24个
...
def save_prepr(args, whitened_test, whitened_train, img_conditions_train,
	...
	save_dir = os.path.join(args.project_dir,
		'Preprocessed_data_2000Hz', 'sub-'+format(args.sub,'02'))
